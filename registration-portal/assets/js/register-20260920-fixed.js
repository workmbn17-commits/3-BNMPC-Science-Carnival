...
function syncDependentFields(){
  ...
  if((fixedTeam || participation.value==="team") && !teamMembers.children.length){
    addMemberCard();
  }
...
function addMemberCard(){
  ...
  d.innerHTML=
    '<div class="member-head"><strong>Team member '+n+'</strong>'+
      (n>1?'<button type="button" class="button ghost small remove-member">Remove</button>':"")+
    '</div>'+
  ...
  if(remove) remove.onclick=()=>{d.remove();renumberMembers();};
...
function renumberMembers(){
  [...teamMembers.children].forEach((card,i)=>{
    ...
    if(remove) remove.style.display=i===0?"none":"inline-flex";
  });
}
